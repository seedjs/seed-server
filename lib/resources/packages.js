...
exports.index = function(req, res) {
  ...
  Token.validate(req, function(err, currentUser) {
    ...
    // finally filter based on if you have access to see a given package
      ...
      Cs.debug('3: found: ' + packages.map(function(p) { return p.id; }).join(','));

      Cs.filter(packages, function(packageInfo, done) {
    ...
    // encode and return
      ...
      Cs.debug('4: found: ' + packages.map(function(p) { return p.id; }).join(','));
...
exports.show = function(req, res, id) {
  ...
  Token.validate(req, function(err, currentUser) {
    ...
    (function(done) {
      ...
      id = Cs.path.join(id, version);
      Cs.debug('finding packageId='+id);
...
exports.install = function(pkg, assetFilename, done) {
  // var info = Cs.mixin({}, pkg.info()); // copy and fixup
  // info['link-asset'] = '/seed/assets/'+assetFilename;
  // info['link-self'] = '/seed/packages/'+pkg.name()+'?version='+pkg.version();
  // 
  // var infoPath = Cs.path.join(server.root, 'packages', pkg.name(), pkg.version() + '.json');
