...
exports.process = function(path, currentUser, done) {
  ...
  Cs.chain(function(done) {
    var unzipdir = path.slice(0, 0-Cs.path.extname(path).length);
    var cmd = 'unzip ' + path + ' -d ' + unzipdir;
    Cs.fs.exec(cmd, function(err) { 
  ...
  function(unzipdir, done) {
    Cs.fs.readdir_p(unzipdir, function(err, dirnames) {
      if (!err && (!dirnames || !dirnames.length===0)) {
        err = "Zipped package is empty";
      }
      if (err) return done(err);
      else return done(null, Cs.path.join(unzipdir, dirnames[0]));
    });
