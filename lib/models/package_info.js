...
var PackageInfo = Record.extend({
  ...
  assetPath: function() {
    return Cs.path.join(server.root, 'assets', this.assetId());    
  },
...
PackageInfo.findAll = function(done) {
  var path = Cs.path.join(server.root, 'packages');
  Cs.fs.glob(path, function(err, filenames) {
    if (err) return done(err);
    if (!filenames) filenames = [];
    Cs.debug(Cs.inspect(filenames));
...
PackageInfo.findCompatible = function(id, done) {
  ...
  var path = Cs.path.join(server.root, 'packages', id);
  
  Cs.fs.glob(path, function(err, versions) {
    if (err) return done(err);
...
PackageInfo.install = function(pkg, srcPath, currentUser, done) {
  ...
  var packageId = Cs.path.join(pkg.name(), pkg.version());
  var assetFilename = pkg.name() + '-' + pkg.version() + '.zip';
  
  Cs.chain(function(done) {
  ...
  function(done) {
    var dstPath = Cs.path.join(server.root, 'assets', assetFilename);

    // remove any existing asset by that name
    Cs.chain(function(done) {
      Cs.fs.exists(dstPath, function(err, exists) {
        if (err || !exists) return done(err);
        Cs.fs.rm_r(dstPath, function(err) { done(err); });
      });
    },
    
    // copy over new asset
    function(done) {
      Cs.fs.mkdir_p(Cs.path.dirname(dstPath), 511, function(err) {
        if (err) return done(err);
        Cs.fs.cp_r(srcPath, dstPath, function(err) { done(err); });
      });
...
PackageInfo.latestVersionFor = function(packageId, done) {
  ...
  Cs.fs.readdir_p(path, function(err, dirs) {
