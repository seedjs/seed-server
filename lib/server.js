...
exports.start = function(opts, done) {
  ...
  exports.root = Cs.path.normalize(exports.root);
      
  Cs.println("Seed Server v" + module.ownerPackage.get('version'));
  Cs.println("Rooted at " + exports.root);
  ...
  // cleanup tmp
  var tmppath = Cs.path.join(exports.root, 'tmp');
  Cs.fs.exists(tmppath, function(err, exists) {
    if (exists) Cs.fs.rm_r(tmppath, Cs.noop);
  });
...
exports.error = function(res, err, desc) {
  ...
  // passing just an error code should just return the error code
  if ('number' === typeof err) {
    res.simpleText(err, desc||'');
  
  // passing a string should match to standard error codes (from couch)
  } else if (('string' === typeof err) && ERRORS[err]) {
    res.simpleText(ERRORS[err], err);

  // otherwise, handle genuine error cases
  } else if (err.status && err.message) {
    res.simpleText(err.status, err.message);

  } else {
    res.simpleText(503, err.toString());
  }
...
exports.forbidden = function(res, reason) {
  return exports.error(res, 403, reason);
};
...
exports.readJson = function(path, done) {
  cache.read(path, function(done) {
    Cs.fs.exists(path, function(err, exists) {
      ...
      if (exists) {
        Cs.fs.readFile(path, function(err, content) {
          if (err) return done(err);
          try {
...
exports.writeJson = function(path, content, done) {
  ...
  Cs.fs.exists(path, function(err, exists) {
    ...
    if (exists) {
      Cs.fs.rm_r(path, function(err) {
        if (err) return resetCache(err);
        Cs.fs.writeFile(path, content, function(err) { resetCache(err); });
      });
      
    // file does not exist - make containing dirs and then write
    } else {

      Cs.fs.mkdir_p(Cs.path.dirname(path), O_RWX, function(err) {
        if (err) return resetCache(err);
        Cs.fs.writeFile(path, content, function(err) { resetCache(err); });
      });
    }
...
exports.sendFile = function(res, path) {
  ...
  var contentType = router.mime.lookupExtension(Cs.path.extname(path));
  Cs.fs.stat(path, function(err, stats) {
    if (err) return server.error(res, err);

    var len = stats.size, pos = 0;
    Cs.fs.open(path, 'r', 0, function(err, fd) {
      if (err) return server.error(res, err);
      ...
      var readNext = function(done) {
        ...
        Cs.fs.read(fd, 4096, pos, 'binary', function(err, data, byteCnt) {
...
exports.receiveFile = function(req, path, done) {
  ...
  function writeChunk() {
    ...
    if (chunk !== undefined) {
      writing = true;
      Cs.fs.write(fd, chunk, null, 'binary', function(err) {
        if (err) return server.error(res, err);
        writing = false;
        endWrite();
        writeChunk();
  ...
  // also, setup fd
  Cs.fs.mkdir_p(Cs.path.dirname(path), 511, function(err) {
    if (err) return cancel(err);
    
    Cs.fs.open(path, 'w+', 511, function(err, newFd) {
      if (err) return cancel(err);
